...
class PokeBallView(ctx : Context) : View(ctx) {
    ...
    data class PBNode(var i : Int, val state : State = State()) {

        private var next : PBNode? = null
        private var prev : PBNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawPokeBall(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : PBNode {
            var curr : PBNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class PokeBall(var i : Int) {

        private val root : PBNode = PBNode(0)
        private var curr : PBNode = root
